...
class PymunkPhysicsEngine:
    ...
    def set_anglular_velocity(self, sprite: Sprite, anglular_velocity: float):
        """ Set anglular_velocity of sprite. Can cause wild behavior at fast changes in angle """
        physics_object = self.get_physics_object(sprite)
        if physics_object.body is None:
            raise PymunkException("Tried to set a anglular_velocity, but this physics object has no 'body' set.")
        physics_object.body.angle = anglular_velocity

    def set_angle(self, sprite: Sprite, angle: float):
        """ Set angle of sprite. Can cause wild behavior at fast changes in angle """
        physics_object = self.get_physics_object(sprite)
        if physics_object.body is None:
            raise PymunkException("Tried to set a angle, but this physics object has no 'body' set.")
        physics_object.body.angle = angle
    ...
    def apply_force_point(self, sprite: Sprite, force: Tuple[float, float], point: Tuple[float, float]):
        """ Apply force to a Sprite. """
        physics_object = self.sprites[sprite]
        if physics_object.body is None:
            raise PymunkException("Tried to apply a force, but this physics object has no 'body' set.")
        physics_object.body.apply_force_at_local_point(force, point)
